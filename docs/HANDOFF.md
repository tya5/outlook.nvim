...
11. **`:OutlookCalendar`(`list_events`/`Invoke-ListEvents`)**:
    - `Restrict`の日付フィルタ(`[Start] < '<.ToString("g")>' AND [End] > '<.ToString("g")>'`)がOutlookのロケール依存パーサと噛み合うか。特に日本語Windows環境で`.ToString("g")`の書式(`yyyy/MM/dd H:mm`等)とOutlookの内部パーサが一致するかは未検証(DESIGN.md 8章9番)。ズレる場合、期間外のイベントが混ざる/正しい範囲が空になるなどの症状になりうる(2026-07-25の`to`境界バグとは別の、書式レベルの懸念として引き続き未検証)。
    - `$items.IncludeRecurrences = $true` による定例会議の展開が期待通り動くか(マスターアイテムではなく個々のオカレンスとして`Start`/`End`が正しく展開されるか)。
    - `ConvertTo-UnixTime`(`DateTime`を「ローカル時刻」とみなして`TimeZoneInfo.Local`でUTC変換)が、Outlookのタイムゾーン設定と実際のWindowsシステムタイムゾーンが異なる環境(例: 出張中で片方だけ変更した場合)でもズレないか。
    - almanac.nvim側の月/週/日ビュー切り替えで`range`が変わるたびに`list_events`が再実行されるが、Outlook予定表の規模によっては`<C-f>`連打等で体感遅延が出るか(mail一覧側で実施したキャッシュ/多重リクエスト抑止に相当する仕組みは、calendar.lua側にはまだ無い)。
