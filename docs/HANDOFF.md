...
10. **`<C-r>`/`<C-f>`の行更新修正、および`<C-n>`(もっと読み込む)**: `refresh_row()`が使う`picker:refresh()`は snacks.nvim のソースコード上で存在は確認したが、実際にその場で行の表示が更新される(カーソルを動かさなくても反映される)かは未確認。`<C-n>`も同様に、`picker:refresh()`が「`Snacks.picker.pick({items=...})`に渡した後で中身を書き換えたテーブル」を正しく再走査して件数が増えて表示されるかが未確認(DESIGN.md 6.2節参照)。どちらも「データは正しく更新されるが表示が追従しない」場合は picker を閉じて開き直せば正しい状態が見える設計なので、最悪でも実害はないはず。
