...
1. `:checkhealth outlook` が all OK。
2. `:OutlookOpen` 初回: helperが起動し一覧が出る(Outlook起動済み/未起動の両方で。未起動時は OUTLOOK_NOT_RUNNING 通知)。
3. New Outlook 環境で明確なエラーになること。
4. 日本語件名・本文が文字化けしないこと(P1-1のエンコーディング周り)。
5. snacks API 前提箇所の実動作: `Snacks.picker.pick` の `format`/`confirm`/`actions`/`win.input.keys`、`Snacks.win({text=...})`・`win:map()`・`win.buf`。
6. 大容量メールボックス(数千通)での一覧レイテンシ。
7. `<C-r>` トグル後の Outlook 本体側の既読状態反映。
8. Object Model Guard プロンプトが read-only 操作で出ないこと。
