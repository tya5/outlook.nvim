...
11. **`:OutlookCalendar`(`list_events`/`Invoke-ListEvents`)**:
    - `Restrict`の日付フィルタ(`[Start] < '<.ToString("g")>' AND [End] > '<.ToString("g")>'`)がOutlookのロケール依存パーサと噛み合うか。特に日本語Windows環境で`.ToString("g")`の書式(`yyyy/MM/dd H:mm`等)とOutlookの内部パーサが一致するかは未検証(DESIGN.md 8章9番)。ズレる場合、期間外のイベントが混ざる/正しい範囲が空になるなどの症状になりうる。
