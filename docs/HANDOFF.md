...
11. **`:OutlookCalendar`(`list_events`/`Invoke-ListEvents`)**: 以下3点は2026-07-25にMicrosoft公式ドキュメントで裏取り調査済み(出典は各項目末尾)。
    ...
    - almanac.nvim側の月/週/日ビュー切り替えで`range`が変わるたびに`list_events`が再実行されるが、Outlook予定表の規模によっては`<C-f>`連打等で体感遅延が出るか(mail一覧側で実施したキャッシュ/多重リクエスト抑止に相当する仕組みは、calendar.lua側にはまだ無い)。これはドキュメント調査で解決できる性質のものではなく引き続き実機確認事項。
