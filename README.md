...
- 件名・差出人での検索
- 「もっと読み込む」(`snacks.picker`使用時、picker内`<C-n>`)で一覧・検索結果を件数上限を増やして再取得(50件ずつ。Outlook COM/snacks.picker双方に増分取得の仕組みが無いため、都度フォルダの先頭から取り直す実装。詳細は[docs/DESIGN.md](docs/DESIGN.md) 6.1節)
...
| `<C-n>` | もっと読み込む(50件ずつ追加取得。取得済み件数が増えない=末尾に達した場合は通知) |
